...
1. KO self to deal high damage (Self-Destruct, Explosion),
2. Deal damage to a sleeping target and restore equivalent HP (Dream Eater),
3. Charge turn with high critical-hit rate (Razor Wind),
4. Charge turn with high critical-hit rate and a chance to flinch the target (Sky Attack),
5. Charge turn with a defense boost (Skull Bash),
6. Charge turn that is skipped during harsh sunlight (Solar Beam),
7. Recharge after use (Hyper Beam, Giga Impact, Frenzy Plant, Blast Burn, Hydro Cannon),
8. Scale base power down as a percentage of current HP (Water Spout, Eruption),
9. Scale base power up if the user is slower than the target (Gyro Ball),
10. Scale base power up based on the target's weight (Low Kick, Grass Knot),
11. Take 50% of the dealt-damage as recoil (Head Smash),
12. Deal damage equivalent to the user's level (Night Shade, Seismic Toss),
13. Deal damage based on friendship (Return, Frustration),
14. Deal a flat amount of damage (Dragon Rage, Sonic Boom),
15. Spit Up,
16. Focus Punch,
17. Superpower,
18. Sucker Punch,
...
6. [Prioritize Extremes Flag](#prioritize-extremes-flag)
...
## Prioritize Extremes Flag

Philosophy: Prioritize moves with variable power, flat damage, high base power, or zero power.

This routine operates against all status moves (which have power 0) and any move-effect which is otherwise ignored by the damage calculation. Refer to the start of this document for a list of such effects.

If the move to be used would not be considered for damage calculation, ~61% of the time, score +2.
